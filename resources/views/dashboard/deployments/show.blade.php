        ...
        <div class="col-md-12">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li {!! $tab != '' ?: 'class="active"' !!}><a href="{{ route('deployments',['project_id'=>$project->id]) }}"><span class="piplin piplin-clock"></span> {{ trans('tasks.label') }}</a></li>
                    <li {!! $tab != 'environments' ?: 'class="active"' !!}><a href="{{ route('deployments',['project_id'=>$project->id, 'tab'=>'environments']) }}"><span class="piplin piplin-environment"></span> {{ trans('environments.label') }}</a></li>
                    <li {!! $tab != 'commands' ?: 'class="active"' !!}><a href="{{ route('deployments',['project_id'=>$project->id, 'tab'=>'commands']) }}"><span class="piplin piplin-command"></span> {{ trans('commands.label') }}</a></li>
                    <li {!! $tab != 'config-files' ?: 'class="active"' !!}><a href="{{ route('deployments',['project_id'=>$project->id, 'tab'=>'config-files']) }}"><span class="piplin piplin-config-file"></span> {{ trans('configFiles.label') }}</a></li>
                    <li {!! $tab != 'shared-files' ?: 'class="active"' !!}><a href="{{ route('deployments',['project_id'=>$project->id, 'tab'=>'shared-files']) }}"><span class="piplin piplin-shared-file"></span> {{ trans('sharedFiles.tab_label') }}</a></li>
                </ul>
