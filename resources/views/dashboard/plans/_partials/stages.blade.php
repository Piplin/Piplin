<div class="box">
    ...
    <div class="box-body table-responsive">
        <table class="table table-striped">
            ...
            <tbody>
                @foreach(['install', 'build', 'finish'] as $index => $stage)
                <tr>
                    <td>{{ $index+1 }}</td>
                    <td>{{ $project->{'before_'.$stage} }}</td>
                    <td><a href="{{ route('commands.step', ['id' => $project->id, 'command' => $stage]) }}">{{ trans('commands.'.$stage) }}</a> <i class="fixhub fixhub-info" data-toggle="tooltip" data-placement="right" data-html="true" data-original-title="{!! trans('commands.'.$stage.'_help') !!}"></i></td>
